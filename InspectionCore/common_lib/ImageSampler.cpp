...
void stageLightParam::nodesIdxWHSetup()
{
  ...
  {
    if(idxW<node.index.X)idxW=node.index.X;
    if(idxH<node.index.Y)idxH=node.index.Y;
  }
  ...
  {
    if(node.mean!=node.mean)continue;
...
{
  if(X<-1 || Y<-1 ||X>=idxW+1 || Y>=idxH+1)return NULL;
  int idx = (X+1)+(Y+1)*(idxW+2);
  if(idx>=BG_exnodes.size())return NULL;
