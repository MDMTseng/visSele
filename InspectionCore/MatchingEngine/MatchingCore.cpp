...
{
  // Bind an acvImage shim over the cv::Mat so getImage() can hand the
  // still-acv internals (contourPixExtraction, pixFetch, EdgePointOpt*,
  // search_point_cv, caliper_locate_*) the type they expect.
  this->_gray_shim.useExtBuffer(graylevelImg_cv.data,
                                (int)(graylevelImg_cv.total() * graylevelImg_cv.elemSize()),
                                graylevelImg_cv.cols, graylevelImg_cv.rows);
...
void ComputeConvexHull_x(const acv_XY *polygon,const int L,std::vector<int> &ret_chIdxs)//The polygon cannot have internal overlap/loop
{
	// The polygon needs to have at least three points
	if (L < 3)
	{
  ...
  int order=1;
	...
	*/
    ...
    float angle=0;
      ...
      acv_XY pnd2=polygon[ret_chIdxs[ret_chIdxs.size() - 2]];
      acv_XY pnd1=polygon[ret_chIdxs[ret_chIdxs.size() - 1]];
      acv_XY pt=polygon[i];


      float cross1= CrossProduct_SS(pnd2, pnd1, pt);
      ...
      if(cross1>0)
      {//ok
        acv_XY pn0=polygon[ret_chIdxs[0]];
        float cross2 = CrossProduct_SS(pnd1,pt, pn0);
