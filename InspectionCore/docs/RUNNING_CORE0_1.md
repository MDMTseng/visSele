...
## `--sbm-train` (debug): regenerate a def's SBM features headlessly

```
visSele --sbm-train <def.hydef> <out.hydef>
```

The command-line twin of the studio's 生成特徵點: extracts line2Dup features from the def's
reference image (`<def>.png` / `reference_image`) inside the same extraction window the SF
handler uses, and writes the def back with the fresh `shape_cache` in `@__SBM_INFO__`. Refuses
to overwrite its input unless `INSP_SBM_TRAIN_INPLACE=1` -- fleet defs are published from one
place (HANDOVER D2); this is for the bench, not for every machine. Exit 0 ok, 2 usage, 3 read,
4 def/extraction failure. Verified: test1 regenerated byte-identical feature counts (235/128).
