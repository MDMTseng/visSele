...
> **2026-09-17,併入 2.0rc2 時補記。** 這份文件是為 `ct/sbm_residual_rc2` 寫的,那條分支的程式碼**沒有**併進來,只有這份文件併了。
> 
> 原因:2.0rc2 後來用更完整的做法解決了同一個問題。文件裡講的 `refine_residual` 在核心裡是以 `trust_residual` 的名字落地的(`FeatureManager_sig360_circle_line.cpp`),JSON 輸出的 key 是 `trust.residual` 而不是 `refine_residual`,而且多了一個這份文件沒有的東西 —— `trust_alt_residual`,同一位置上第二好姿態的殘差,也就是這次定位有多模糊。
> 
> 所以把那 45 行程式碼合進來只會多出一個和 `trust.residual` 量同一件事的平行欄位,而 WebUI 讀的是 `trust.*`。
> 
> **文件本身留下來的理由是量測數據和那段自我檢討**,兩者都不會因為實作換了名字而失效。
