...
// Build provenance (git hash + build time), generated by cmake/gen_build_info.cmake
// into the build dir. Guarded so the TU still compiles if it hasn't been generated.
...
#endif
// Build time = this TU's compile time. Kept out of build_info.h so a changing
// timestamp doesn't force a relink every build.
...
static void log_request_dump_marker(uint32_t marker) {
    std::lock_guard<std::mutex> lk(g_log_mutex);
...
void log_request_shutdown_dump(void) {
    log_request_dump_marker(LOG_CRASH_SHUTDOWN);
}
