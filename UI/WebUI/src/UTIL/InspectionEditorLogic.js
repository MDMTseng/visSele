...
export class InspectionEditorLogic {
  ...
  SetDefInfo(defInfo) {
    ...
    // The localization pair stays off. Its include polygon is usually the BAKED
    // sig360 silhouette -- hundreds of vertices generated by the migration, not
    // drawn by anyone -- and rebuilding it puts an uneditable blob on the canvas
    // over every measurement feature. It survives a save without being rebuilt
    // because defFileGeneration re-bakes it from the signature.
    ...
    // The FENCE is rebuilt, and has to be. It is always hand-drawn, it is small,
    // and nothing re-derives it: defFileGeneration emits measure_fence_* from
    // these shapes and from nothing else, so a fence that is not rebuilt here is
    // a fence the operator silently loses on the next save.
    addRegionShapes(defInfo.measure_fence_include, 'fence_include', '@__FENCE_INCLUDE__');
    addRegionShapes(defInfo.measure_fence_exclude, 'fence_exclude', '@__FENCE_EXCLUDE__');
