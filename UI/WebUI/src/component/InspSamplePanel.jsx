...
//
// The buffer behind it (UTIL/inspSampleRing) is a SAMPLE: only parts whose
...
import {
  SAMPLE_RING_BUCKETS, sampleRingSnapshot, subscribeSampleRing,
  clearSampleRing, sampleRingCap, setSampleRingCap,
} from 'UTIL/inspSampleRing';
...
function Detail({ entry, url, onSave, saving }) {
...
export function InspSamplePanel({ visible, onClose, sendBPG, savePath, defInfoFor }) {
  ...
  useEffect(() => subscribeSampleRing(() => setTick((t) => t + 1)), []);
  const snap = useMemo(() => sampleRingSnapshot(), [tick, visible]);
  const entry = sel !== undefined
    ? SAMPLE_RING_BUCKETS.map((b) => snap[b].find((e) => e.id === sel))
        .find((e) => e !== undefined)
    : undefined;
  ...
  const total = SAMPLE_RING_BUCKETS.reduce((n, b) => n + snap[b].length, 0);
  ...
  return <Modal open={visible} visible={visible} onCancel={onClose} footer={null}
    ...
    <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 8 }}>
      ...
      <InputNumber size="small" min={1} max={500} value={sampleRingCap()}
        onChange={(v) => { setSampleRingCap(v); setTick((t) => t + 1); }} style={{ width: 80 }} />
      <span style={{ fontSize: 12, color: '#888' }}>
        共 {total} 筆,{(snap.bytes / 1048576).toFixed(1)} MB
      </span>
      <Button size="small" icon={<DeleteOutlined />}
        onClick={() => { clearSampleRing(); setSel(undefined); }}>清空</Button>
    ...
    <div style={{ display: 'flex', gap: 12 }}>
      <div style={{ display: 'flex', gap: 8, width: 470 }}>
        {SAMPLE_RING_BUCKETS.map((b) => (
          <div key={b} style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: COLOUR[b], fontWeight: 600, marginBottom: 4 }}>
              {LABEL[b]} <span style={{ color: '#888', fontWeight: 400 }}>{snap[b].length}</span>
            </div>
                ...
                : snap[b].map((e) => (
                  <div key={e.id} onClick={() => setSel(e.id)}
                    ...
                    <div style={{ fontSize: 10, color: '#888',
                                  fontVariantNumeric: 'tabular-nums' }}>{hhmmss(e.at)}</div>
                  </div>))}
            </div>
          </div>))}
      </div>
