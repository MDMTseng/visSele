...
const ws=new WebSocket('ws://127.0.0.1:4090');ws.binaryType='arraybuffer';let pg=1,rp=0,im=0;
ws.on('message',d=>{const b=new Uint8Array(d);const t=String.fromCharCode(b[0],b[1]);
 if(t==='HR'){ws.send(frame('HR',0,pg++,{a:['d']}));ws.send(frame('SB',0,pg++,{sub:true}));return;}
 if(t==='RP')rp++; if(t==='IM')im++;
});
const secs=Number(process.argv[2]||15);
setTimeout(()=>{console.log(`watched ${secs}s: RP=${rp} IM=${im}`);process.exit(0);},secs*1000);
