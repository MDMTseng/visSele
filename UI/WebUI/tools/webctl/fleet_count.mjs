...
// Runs every recipe listed in _ok_names.txt (data/_test/<name>_sbm.hydef, or the sig360 original with -sig) on its own
// data/_test/<name>.png once and prints located / judges-OK totals; per-recipe counts go to $OUT (default _fleet_count.json).
...
for (const n of names) { const def=JSON.parse(fs.readFileSync(D+n+suffix+'.hydef','utf8')); const rp=await ii(def,'data/_test/'+n+'.png'); const g=rp&&rp.reports&&rp.reports[0]; const o=g&&g.reports&&g.reports[0];
