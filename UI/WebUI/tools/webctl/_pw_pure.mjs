// Overlay off ("純圖"), then hash the picture. With the overlay drawn on top,
// a stale photo and a live one are indistinguishable: the measurements are
// redrawn every report and the core skips IMAGES but never REPORTS
// ("skipped img:N rep:0"), so fresh numbers on an old frame look exactly like
// a live view. 純圖 removes the only thing that was proving it.
...
const pure = await page.evaluate(()=>{
  for(const e of document.querySelectorAll('*')){
    ...
    if((e.innerText||'').trim()!=='純圖') continue;
    const sw = e.parentElement && e.parentElement.querySelector('.ant-switch,button[role=switch]');
    (sw||e).click(); return sw? 'switch clicked':'label clicked';
  } return 'not found';});
console.log('純圖:',pure);
await sleep(4000);
...
for(let i=0;i<6;i++){
  ...
  hs.push(crypto.createHash('md5').update(png).digest('hex').slice(0,10));
  if(i<3) await page.screenshot({path:`${OUT}/pure_${i}.png`, clip:c});
  await sleep(1500);
}
console.log('hashes:',hs.join(' '));
console.log([...new Set(hs)].length>1 ? 'PHOTO IS LIVE' : 'PHOTO IS FROZEN (only overlays were changing)');
await browser.close();
