...
| `cat` | 行為 | 計數器 |
|---|---|---|
| `1` | 觸發 SEL1 | `SEL1_Count` |
| `2` | 觸發 SEL2 | `SEL2_Count` |
| `3` | **不觸發任何氣閥**（SEL3 未實作，見 §5.4）| `SEL3_Count` |
| `0xFFFF` (65535) | **NA —— 不觸發氣閥，料件直接流過** | `NA_Count` |
| `insp_status_SKIP` (-2100) | 跳過 | `SKIP_Count` |
| 其他 / 未設定 | **`OBJECT_HAS_NO_INSP_RESULT` → 進錯誤態停機** | — |

> **掉幀要回報 `cat=0xFFFF`（NA），不是 NG。**
> NG（`cat=2`→SEL2）的語意是「檢測過且不合格」；掉幀是「根本沒檢測到」。
> 兩者混用會污染 NG 統計、讓良率數據失真，也讓真正的品質問題被雜訊蓋掉。
> NA 不觸發氣閥，料件流向預設路徑。
...
⚠ **掉幀一定要走 NA（`cat=0xFFFF`）而不是 NG。** 見 §4 的 cat 值表：NG 的語意是
「檢測過且不合格」，掉幀是「沒檢測到」。判成 NG 會把驅動器/USB 的傳輸問題
混進良率統計裡，之後追品質問題時會被這些雜訊帶偏。
