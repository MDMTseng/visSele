...
GSCALE=0.01
modelvbowlId=LoadObj("data/Untitled-62aab7cd.stl",0,[0,0,0],p.getQuaternionFromEuler([0,0,math.pi*(15)/180]), [-0.02, -0.02, -0.005],GSCALE,[0.7, 0.7, 0.7, 1])
...
while True:
  ...
  valpha=p.readUserDebugParameter(vibSlider_id)
  respawnSep=6
  for sid in sidArr:
    sPos, sOrn = p.getBasePositionAndOrientation(sid)
    if(portalCD==0 and sPos[2]<0.2):
      sPos=[-0.90,-0.3+(0.1*(totPortalCount%respawnSep)),0.5]
    ...
    RAlpha=1
    CWAlpha=3.5
    x=(RAlpha*x+CWAlpha*nx)*valpha/vibVDefault*subStep
    y=(RAlpha*y+CWAlpha*ny)*valpha/vibVDefault*subStep
